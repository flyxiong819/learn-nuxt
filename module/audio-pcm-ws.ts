...
export class MyAudio {
  ...
  async start(examTypeStr: string, callback: (text: string) => void) {
    // -------------------后面抽取出去start-------------
    // 清理之前的websocket
    if (this.ws) {
      try {
      ...
      if (typeof e.data === "string") {
        try {
          ...
          if (m.event === "partial") {
            // 实时识别的中间结果
            callback(m.text);
          } else if (m.event === "llm_complete") {
            // 最终识别结果
            callback(m.text);
          } else if (m.event === "ready") {
            await this.startAudio();
            console.log("✅ ASR已就绪 - 现在可以开始说话");
          } else if (m.event === "end") {
            console.log("识别结束");
            setTimeout(() => {
              try {
    ...
    this.ws.onerror = (error: any) => {
      console.log("WebSocket 错误: " + (error.message || "连接失败"));
      this.stopAudio();
    };
    // -------------------后面抽取出去end-------------
